...
export default function SlideBottom (props: any): React.JSX.Element {
  ...
  useEffect(() => { fadeAnim.setValue(isOpenWeatherForecast ? state.openTop : state.closeTop) }, []);
  ...
  const onPanMove = (e) => {
    if(isOpenWeatherForecast){
      ...
      fadeAnim.setValue(state.openTop + e.translationY)
    } else {
      if(e.translationY > 0){
        e.translationY = 0
      }
      fadeAnim.setValue(state.closeTop + e.translationY)
    }
